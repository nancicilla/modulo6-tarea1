var app=new Vue(
    {
        ...
        methods:{
            ...
            seleccionarProducto:function(idproducto){
                this.pedido.id=idproducto;
                let seleccionado= this.productosRelacionados.filter( (x) =>x.id==idproducto);
                seleccionado=seleccionado[0];
                let productoactual = Object.assign({}, this.producto);                
                if(productoactual.id==null)
                {
                    productoactual.id=1;
                }
                this.producto.id=seleccionado.id;
                this.producto.nombre=seleccionado.nombre;
                this.producto.imagen=seleccionado.imagen;
                this.producto.precio=seleccionado.precio;
                this.producto.descripcion=seleccionado.descripcion;
                this.producto.colores=seleccionado.colores;

                indice=this.productosRelacionados.findIndex((x)=>x.id==idproducto);
                this.productosRelacionados.splice(indice,1,productoactual);
                this.pedido.color=null;
